...
$categories = @("תפילין וסת\"ם", "מזוזות ובית", "טליתות וציציות", "יודאיקה ומתנות", "ארנקים ותיקים", "חגים ומועדים", "ספרי תורה")
...
for ($i = 0; $i -lt 45; $i++) {
    $id = $i + 1
    $title = $titles[$i]
    $catIndex = $i % $categories.Length
    $cat = $categories[$catIndex]
    if ($title -like "*ארנק*" -or $title -like "*תיק*") { $cat = "ארנקים ותיקים" }
    elseif ($title -like "*תפילין*" -or $title -like "*סת\"ם*" -or $title -like "*כתב*") { $cat = "תפילין וסת\"ם" }
    elseif ($title -like "*מזוז*") { $cat = "מזוזות ובית" }
    elseif ($title -like "*טלית*" -or $title -like "*ציצית*") { $cat = "טליתות וציציות" }
...
$json = $articlesList | ConvertTo-Json -Depth 5

$jsCode = "const disclaimerHtml = `$disclaimer;` `n`nconst articlesDB = $json;"

[System.IO.File]::WriteAllText($outPath, $jsCode, $utf8)

Write-Host "✅ Generated 45 clean UTF-8 articles in articles_data.js!"
