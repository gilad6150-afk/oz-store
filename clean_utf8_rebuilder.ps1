...
$layoutTxt = [System.IO.File]::ReadAllText($layoutPath, $utf8)

# Strip any Mojibake keywords & description
if ($layoutTxt.Contains('׳ž')) {
    $cleanMeta = @"
...
"@
    $targetStr = '<!-- OFFICIAL BRAND FAVICON -->'
    $layoutTxt = $layoutTxt.Replace($targetStr, $cleanMeta + "`n    " + $targetStr)
    [System.IO.File]::WriteAllText($layoutPath, $layoutTxt, $utf8)
    Write-Host "Cleaned layout_frame.html!"
}

# 2. Re-read all templates and compile index.html
...
Write-Host "Rebuilt all site targets with 100% clean UTF-8 encoding!"
