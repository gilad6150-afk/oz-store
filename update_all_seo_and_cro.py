...
with open('modals.html', 'r', encoding='utf-8') as f:
...
if 'id="vip-coupon-modal"' not in modals_content:
    modals_content += vip_modal_html
    with open('modals.html', 'w', encoding='utf-8') as f:
        f.write(modals_content)
    print("Updated modals.html with VIP Coupon Modal!")

# 2. Add VIP functions & Schema Microdata to clean_render.js
with open('clean_render.js', 'r', encoding='utf-8') as f:
    clean_render_code = f.read()

# Add VIP helper functions if not present
vip_js_functions = '''
...
if 'window.initVipCouponPopup' not in clean_render_code:
    clean_render_code += vip_js_functions

# Update renderProducts in clean_render.js to include itemscope itemtype Product & Schema microdata
old_card_pattern = '''return `<div class="bg-white border border-slate-100 rounded-2xl sm:rounded-3xl overflow-hidden oz-shadow group flex flex-col justify-between transition-transform duration-300 hover:-translate-y-1 relative">'''
new_card_html = '''return `<div itemscope itemtype="https://schema.org/Product" class="bg-white border border-slate-100 rounded-2xl sm:rounded-3xl overflow-hidden oz-shadow group flex flex-col justify-between transition-transform duration-300 hover:-translate-y-1 relative">
...
if old_card_pattern in clean_render_code:
    clean_render_code = clean_render_code.replace(old_card_pattern, new_card_html)
    print("Replaced product card in clean_render.js with Schema Microdata!")

# Update product card category & stars display
old_cat_stars = '''<div class="text-[9px] sm:text-[10px] font-extrabold text-oz-primary uppercase mb-0.5 sm:mb-1 truncate">${catName}</div>'''
new_cat_stars = '''<div class="flex items-center justify-between mb-0.5 sm:mb-1">
...
if old_cat_stars in clean_render_code:
    clean_render_code = clean_render_code.replace(old_cat_stars, new_cat_stars)
    print("Added gold star ratings to product cards in clean_render.js!")

with open('clean_render.js', 'w', encoding='utf-8') as f:
    f.write(clean_render_code)

print("Updated clean_render.js successfully!")
