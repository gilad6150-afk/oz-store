...
clean_nav_path = r"C:\Users\97254\.gemini\antigravity\scratch\oz-store\clean_nav.html"
with open(clean_nav_path, 'r', encoding='utf-8') as f:
    clean_nav = f.read()

# Replace "בדיקה מוסמכת במקום"
clean_nav = clean_nav.replace('בדיקה מוסמכת במקום', 'בדיקה מוסמכת ומחשב')

new_nav = '''                <!-- NAVIGATION LINKS WITH SLEEK MEGA MENU DROPDOWNS FOR ALL CATEGORIES (CENTERED) -->
...
clean_nav = re.sub(r'<!-- NAVIGATION LINKS WITH SLEEK MEGA MENU DROPDOWNS FOR ALL CATEGORIES -->[\s\S]*?<!-- HEADER ACTIONS & ICONS', new_nav + '\n\n                <!-- HEADER ACTIONS & ICONS', clean_nav)

with open(clean_nav_path, 'w', encoding='utf-8') as f:
    f.write(clean_nav)

# Update shop_layout.html & index.html for "במקום" text
shop_layout_path = r"C:\Users\97254\.gemini\antigravity\scratch\oz-store\shop_layout.html"
with open(shop_layout_path, 'r', encoding='utf-8') as f:
    sl = f.read()
sl = sl.replace('שירות בדיקת תפילין ומזוזות במקום', 'שירות בדיקת תפילין ומזוזות מוסמכת')
with open(shop_layout_path, 'w', encoding='utf-8') as f:
    f.write(sl)

index_path = r"C:\Users\97254\.gemini\antigravity\scratch\oz-store\index.html"
...
# Update domain in index.html JSON-LD
idx = idx.replace('ozonlineshop.com', 'oz-judaica.co.il')
with open(index_path, 'w', encoding='utf-8') as f:
    f.write(idx)

print("Updated header nav and clean replacements successfully!")
