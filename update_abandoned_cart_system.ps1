$pagesCodePath = "C:\Users\97254\.gemini\antigravity\scratch\oz-store\pages_code.js"
$utf8 = New-Object System.Text.UTF8Encoding($false)
$content = [System.IO.File]::ReadAllText($pagesCodePath, $utf8)
...
if (-not $content.Contains("window.trackAbandonedCart")) {
    $content += "`n" + $abandonedCartCode
}

if ($content.Contains("window.handleCompleteOrder = function(e) {")) {
    $content = $content.Replace(
        "window.handleCompleteOrder = function(e) {",
        "window.handleCompleteOrder = function(e) {`n            if (typeof clearAbandonedCartOnOrder === 'function') clearAbandonedCartOnOrder();"
    )
}

if ($content.Contains('id="checkout-name" required')) {
    $content = $content.Replace(
...
if ($content.Contains('id="checkout-phone" required')) {
    $content = $content.Replace(
...
$crmTableMarker = "container.innerHTML = html;"
$crmAbandonedCode = @"
...
if (-not $content.Contains("ABANDONED CARTS SECTION") -and $content.Contains($crmTableMarker)) {
    $content = $content.Replace($crmTableMarker, $crmAbandonedCode)
}

[System.IO.File]::WriteAllText($pagesCodePath, $content, $utf8)
Write-Host "SUCCESS: Updated pages_code.js with Abandoned Cart Recovery System!"
