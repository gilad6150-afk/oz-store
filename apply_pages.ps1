...
# 1. Update Cart Checkout Button to trigger openCheckoutModal()
$content = $content.Replace('onclick="alert(''מעבר לתשלום מאובטח בקליק!'')"', 'onclick="toggleCartDrawer(); openCheckoutModal();"')

# 2. Add Floating Accessibility Widget & Modals before </footer>
$modalsHtml = @"
...
$content = $content.Replace('<footer id="footer"', $modalsHtml + "`n`n    <footer id=\"footer\"")
...
# 4. JS Functions for Accessibility, Checkout Modal & Order Completion
$jsCode = @"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                    ...
                    <form onsubmit="handleCompleteOrder(event)" class="space-y-4">
                        ...
                        <button type="submit" class="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm rounded-2xl shadow-xl shadow-emerald-600/20 transition-all active:scale-95 flex items-center justify-center gap-2">
                            <span>אישור הזמנה ותשלום מאובטח 🔒 (₪` + (total >= 399 ? total : total + 35) + `)</span>
                        </button>
                    </form>
...
$content = $content.Replace('function initApp()', $jsCode + "`n`n        function initApp()")

# Save UTF-8 clean
[System.IO.File]::WriteAllText($path, $content, [System.Text.Encoding]::UTF8)
Write-Output "Successfully updated index.html with Checkout Page, Shipping, Terms, Accessibility Widget & Footer Navigation!"
